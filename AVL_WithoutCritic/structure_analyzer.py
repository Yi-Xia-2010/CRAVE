...
def get_latest_outline(base_dir="results"):
    """Find the latest generated outline JSON file (excluding corrected versions)"""
...
def find_target_by_priority(parent_id, target_info, id_map, existing_connections):
    """
    Find target node by priority sequence.
    Priority: Full Title -> ID -> Cleaned Title -> Branch + Keyword -> Branch
    Returns: (target_id, match_reason) or (None, "No match found")
    """
...
def extract_emotion_tag(title):
    """Extract emotion tag [Rise]/[Fall] from title"""
    if not title:
        return None
...
def extract_title_without_emotion(title):
    """Remove emotion tag [Rise]/[Fall] from title"""
    if not title:
        return ""
...
def find_all_paths(graph, start_node):
    """DFS path finding (with simple cycle prevention)"""
    routes = []
...
def compute_connection_quality(parent_node, child_node, target_info):
    """
    Calculate connection quality score (0-10)
    Higher score means the connection is more likely to be correct
    """
    score = 0
...
def detect_all_cycles(children_map):
    """
    Detect all types of cycles (not just self-loops and direct back-loops)
    Uses improved DFS to fix path indexing issues
    """
    cycles_found = []
    visited = set()
...
def break_cycles_intelligently(children_map, id_map):
    """
    Intelligently break cycles, prioritizing retention of higher quality connections.
    Fixed path index issues during cycle detection.
    """
...
def analyze_structure(outline_data):
    """
    Analyze structure and repair source data.
    Includes: Smart connection validation, multi-dimensional repair, cycle detection, orphan removal.
    """
