...
class StoryJudge:
    ...
    def judging(self,story: list, story_type, genre, id, script_name = "not needed", Valance = "Not needed", Arousal= "Not needed" ):
        try:
            api_key = get_api_key()
            ...
            if role == "STORY OUTLINE":
                ...
                sys_prompt = self.criteria
                user_prompt_temp = self.user_prompt_template
                user_prompt = user_prompt_temp.format(game_theme = game_theme, genre = genre, valance = Valance, arousal = Arousal, story_order = story_order) # as output it should return a single value "index position" as 1,2 or 3.
